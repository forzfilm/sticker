...
class ApplyMakeup(DetectLandmarks):
    ...
    def __fill_lip_solid(self, outer, inner,min_inner_color,max_inner_color,min_outer_color,max_outer_color):
        ...
        for point in inner_curve:
            if point[0] <= np.median(inner[0]):
                ...
                cv2.fillPoly(self.image, [points_np], (color_red[i],color_green[i],color_blue[i]))
            i += 1
        self.__fill_lip_solid_outer(outer, inner,min_inner_color,max_inner_color,min_outer_color,max_outer_color)

    def __fill_lip_solid_outer(self, outer, inner,min_inner_color,max_inner_color,min_outer_color,max_outer_color):
        """ Fills solid colour inside two outlines. """
        inner[0].reverse()
        inner[1].reverse()
        outer_curve = zip(outer[0], outer[1])
    ...
    def apply_lipstick(self, filename, rlips, glips, blips,position):
        ...
        if position == "upper":
            self.__fill_color_upper(uol_c, uil_c, lol_c, lil_c,"#FF0009","#FF0009","#FF0009","#EF58FF")
        elif position == "bottom":
            self.__fill_color_bottom(uol_c, uil_c, lol_c, lil_c,"#FF0009","#FF0009","#FF0009","#EF58FF")
        else:
